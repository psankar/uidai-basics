...
All the authentications performed by a Sub-AUA, AUA/KUA, ASA, etc. need to be logged at each level and maintained, for compliance. The fingerprints however should not be stored. It is unclear if/when there will be audits for these logs and what will be the reaction for non-compliance. All devices are servers are expected to be time-sync'd. IST is the only supported timezone. All requests carry a timestamp (naive timestamp as timezone is always IST). A request which has been received by an authentication server where the timestamp is older than a stipulated duration (currently 24 hours) will be rejected. Requests made with timestamps ahead of the actual server timestamp may also be rejected. Additionally, resubmission of the exact request again also results in an error. Hence, a resubmission will have to be in the form of a new request.
